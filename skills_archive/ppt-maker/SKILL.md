...
1. **State Persistence**: If this is a new PPT, you MUST first write the presentation structure to a physical JSON file in the user's workspace (e.g., `~/workspace/docs/sprint_assets/defense_state.json`) using file editing tools. If the user asks for a partial edit, you MUST read this JSON file, modify only the requested parts (e.g., using `replace_file_content`), and save it.
...
```bash
python skills/ppt-maker/gen_ppt.py --input "~/workspace/docs/sprint_assets/defense_state.json" --output "~/workspace/docs/sprint_assets/defense.pptx"
```
