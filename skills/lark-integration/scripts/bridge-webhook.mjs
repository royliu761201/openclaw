...
const APP_SECRET_PATH = resolve(
  process.env.FEISHU_APP_SECRET_PATH || "~/.openclaw/secrets/feishu_app_secret",
);
const OPENCLAW_CONFIG_PATH = resolve(
  process.env.OPENCLAW_CONFIG_PATH || "$HOME/workspace/config/openclaw_core.json",
);
...
const server = http.createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(
      JSON.stringify({ status: "ok", appId: APP_ID, supportedTypes: ["text", "post", "image"] }),
    );
    return;
  }
