...
In `$HOME/workspace/config/openclaw_core.json`:
...
```bash
cat $HOME/workspace/config/openclaw_core.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat $HOME/workspace/config/openclaw_core.json | jq '.gateway.bind'`
