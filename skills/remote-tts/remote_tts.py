...
async def run_remote_tts():
    ...
    cmd = f"nohup sh -c 'curl --noproxy \"*\" -s -w \"\\n%{{http_code}}\" -X POST http://127.0.0.1:8100/synthesize -H \"Content-Type: application/json\" -d @{remote_payload_path} --output {remote_out_path} > {remote_status_path} 2>&1' > /dev/null 2>&1 &"
