...
3. **Execute the Renderer**: Use the `run_command` tool to execute the `render_architecture.py` script. Pass your DOT code snippet inside quotes. **Always save the output to the user's authorized workspace directory** (e.g., `~/Documents/projects/openclaw/workspace/route_map.png`).

```bash
# Example Execution
python skills/diagram-generator/render_architecture.py --output "~/Documents/projects/openclaw/workspace/llm_arch.png" --code 'digraph G { rankdir=LR; User -> API; API -> "LLM Engine"; "LLM Engine" -> Database; }'
```
