...
> 6. **Tailscale PMTUD Blackhole Law (Large Asset Downgrade)**: When pushing large assets (>50MB) across Tailscale nodes (100.x.x.x), standard `ssh_upload` (scp/paramiko) WILL face silent connection drops (`stalled`) due to Path MTU Discovery blackholes (MTU 1280 vs 1500). **YOU MUST ABANDON `ssh_upload`** for these payloads and instead use native `rsync -avz --progress` to forcibly stream. **[WARNING]**: If nodes are on a high-speed direct connect (10.x.x.x/18.x.x.x), **REMOVE THE `-z` FLAG** to prevent CPU-bound throughput collapse.
...
> 15. **The rsync-over-scp Law (Large File Mandate)**: For files >1GB, you are **STRICTLY PROHIBITED** from using `scp`. Use `rsync --partial --progress -e "ssh ..."` exclusively. `scp` has zero resume capability. For high-speed internal links, always append `-o Compression=no` and use a fast cipher (`aes128-gcm`).
> 16. **The High-Speed Network Compression Ban (Internal Sprint)**: On networks with bandwidth >1Gbps (Direct Connect/Local Subnet), **NEVER USE `rsync -z`**. Compression makes the transfer CPU-bound (99% usage) and limits speed to ~6MB/s. Disabling compression and using a fast cipher can boost throughput to **200MB/s+**.
