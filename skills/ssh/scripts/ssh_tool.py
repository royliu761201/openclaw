...
def get_client():
    ...
    # Parse ~/.ssh/config for ProxyCommand
    ...
    def try_connect(pkey=None, pwd=None):
        ...
        if os.path.exists(ssh_config_file):
            ...
            cfg_host = host_config.get('hostname', host)
            cfg_port = int(host_config.get('port', port))
        ...
        try:
            client.connect(cfg_host, port=cfg_port, username=user, pkey=pkey, password=pwd, sock=sock, timeout=10)
            return True
    ...
    # 3. Try Paramiko Default Keys / SSH Agent
    print("Attempting default SSH agent/keys...")
    if try_connect(): return client

    # 4. Try ~/.ssh/id_ed25519 explicitly
    ed_path = os.path.expanduser("~/.ssh/id_ed25519")
    if os.path.exists(ed_path):
        print(f"Attempting {ed_path} explicitly...")
        try:
            k = paramiko.Ed25519Key.from_private_key_file(ed_path)
            if try_connect(pkey=k): return client
        except:
            pass
...
def exec_command(args):
    ...
    print("Got SSH client successfully.")
