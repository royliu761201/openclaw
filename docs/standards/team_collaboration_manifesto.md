...
- **零驻留保障**：所有 ASR/TTS 等资源密集型任务，执行完毕后必须确信显存/内存已 100% 释放。
...
2.  **讨论 (Internal Review)**：
    ...
    - **Dev** 评估技术可行性与接口方案。
    - **QA** 制定暴力测试与 UAT 验收标准。
3.  **自动通过**：若各角色在 5 分钟内无 `BLOCKED` 反馈，则视为达成“初始共识”，立即转入全线执行，无需二次请示。
4.  **汇报共识**：任务通过后，QA 更新 `walkthrough.md`，由 Lead 统一呈报老板终审。
