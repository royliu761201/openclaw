#!/usr/bin/env node
import { Q as isRootVersionInvocation, Y as hasHelpOrVersion, Z as isRootHelpInvocation, t as createSubsystemLogger } from "./subsystem-kzdGVyce.js";
...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-Bxx5bg6l.js").then((n) => n.r), import("./git-commit-Z4N0ntjy.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-Df_wDaVh.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-HKLlbPHn.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
